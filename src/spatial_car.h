#ifndef SPATIAL_CAR_E
#define SPATIAL_CAR_E
...
inline Rcpp::List MH_spatial_correlation_CAR_fast(
    const Eigen::MatrixXd& Z,
    const double& rho1,
    const std::vector<Eigen::SparseMatrix<double>>& Q0,
    const Eigen::VectorXd& allowed_range,
    const Eigen::VectorXd& logdet
);
inline double posterior_conditional_variance(
    const Eigen::MatrixXd& Z,
    const Eigen::SparseMatrix<double>& invCorrZ,
    double a1,
    double b1,
    int nstar,
    int tstar
);
inline Rcpp::List logdet_and_Q0_CAR(
...
inline double MH_spatial_correlation_CAR(
  ...
  for (int i = 0; i < nr; ++i) {
    ...
    Eigen::SparseMatrix<double> invS_unscaled = D - range * W;
    invS_unscaled = 0.5 * (invS_unscaled + Eigen::SparseMatrix<double>(invS_unscaled.transpose()));
...
//--------------------------------------------------------------------------
// Function to sample spatial correlation parameter for CAR model (fast version)
//--------------------------------------------------------------------------
inline Rcpp::List MH_spatial_correlation_CAR_fast(
    const Eigen::MatrixXd& Z,
    const double& rho1,
  ...
  for (int i = 0; i < nr; ++i) {
    // Eigen::SparseMatrix<double> invS_unscaled = Q0[i];
    // Eigen::MatrixXd invS_dense = Eigen::MatrixXd(invS_unscaled) * (1.0 / rho1);
    Eigen::SparseMatrix<double> invS = Q0[i] * (1.0 / rho1);

    double quad_form = 0.0;
    for (int t = 0; t < T; ++t) {
      quad_form += Z.col(t).transpose() * invS * Z.col(t);
    }
...
inline double posterior_conditional_variance(
  ...
  // Quadratic form sum
  // Eigen::MatrixXd invCorrZ_dense(invCorrZ);
  double quad_sum = 0.0;
  for (int t = 0; t < T; ++t) {
...
inline Rcpp::List logdet_and_Q0_CAR(
  ...
  Eigen::VectorXd allowed_range = Eigen::VectorXd::LinSpaced(15, min_tau, max_tau);
  const int nr = allowed_range.size();
  std::vector<Eigen::SparseMatrix<double>> Q0(nr);
  ...
  for (int i = 0; i < nr; ++i) {
    ...
    invS_unscaled = 0.5 * (invS_unscaled + Eigen::SparseMatrix<double>(invS_unscaled.transpose()));
    Q0[i] = invS_unscaled;
    Eigen::MatrixXd invS_dense = Eigen::MatrixXd(invS_unscaled);
